	...
	<script type="text/javascript">
		...
		$("#jq-btn5").on("click", function() {
			...
			$.ajax({ // 서버로 전송값 보내기
				...
				type : "GET",
				success : function(result) {
					$("#p5").html(result.memberId + " : " + result.memberPw);
				},
				error : function() {
					alert("서버 처리 실패");
				}
		...
		document.querySelector("#jq-btn3").addEventListener("click",
				function() {
					$.ajax({
						url : "/ajax/third",
						/* 받을때는 data가 필요없다. 전송시만 필요 */
						/* data : {}, */
						type : "GET",
		...
		function jsFuncPost() {
			...
			xHttp.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					// status => 200(요청 성공), 404(page, url 없음)
					// 500(서버 오류 발생), 403(접근거부), 400(쿼리스트링 오류)
					alert("서버 전송 성공");
